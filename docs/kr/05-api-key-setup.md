...
1. 왼쪽 메뉴에서 **Virtual Keys**를 클릭합니다.

2. **+ Create New Key** 버튼을 클릭합니다.

![LiteLLM Virtual Keys 화면](../../images/capture%201.png)

### 1-3. 키 정보 입력

**Key Ownership** 섹션:
- **Owned By**: `You` 선택 (기본값)
- **Team**: 팀이 있다면 선택, 없으면 비워둠

**Key Details** 섹션:
1. **Key Name** (필수): 키 이름 입력
   - 예: `my-claude-key`, `research-team-key`
   - 나중에 구분하기 쉬운 이름을 사용하세요

2. **Models** (필수): 사용할 모델 선택
   - `claude-opus-4.5` - 최고 성능, 복잡한 작업
   - `claude-sonnet-4.5` - 균형 잡힌 성능
   - `claude-haiku-4.5` - 빠른 응답, 간단한 작업

   > **팁**: 여러 모델을 동시에 선택할 수 있습니다. 일반적으로 세 모델 모두 선택하는 것을 권장합니다.

3. **Key Type**: `Default` (기본값 유지)

![키 생성 화면](../../images/capture%202.png)

### 1-4. 키 생성 완료

1. **Create Key** 버튼을 클릭합니다.

2. 생성된 API 키가 화면에 표시됩니다:
   ```
   sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   ```

3. ⚠️ **중요**: 이 키는 **한 번만 표시**됩니다!
   - 반드시 안전한 곳에 복사해두세요
   - 메모장에 임시로 저장하고, 설정 완료 후 삭제하세요

### 서버 주소 정보

| 항목 | 값 |
|------|-----|
| **API 키** | `sk-xxxxx...` (방금 생성한 키) |
